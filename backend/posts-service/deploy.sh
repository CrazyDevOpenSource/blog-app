...
kubectl apply -f ./devOps/posts-service-deployment.yaml
kubectl apply -f ./devOps/posts-service-service.yaml

# Step 3: Wait for the service to get an external IP
echo "Waiting for the LoadBalancer to get an external IP..."
while true; do
  EXTERNAL_IP=$(kubectl get svc posts-service -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
  if [ -z "$EXTERNAL_IP" ]; then
    # If LoadBalancer IP isn't assigned, fallback to NodePort
    NODE_PORT=$(kubectl get svc posts-service -o jsonpath='{.spec.ports[0].nodePort}')
    if [ -n "$NODE_PORT" ]; then
